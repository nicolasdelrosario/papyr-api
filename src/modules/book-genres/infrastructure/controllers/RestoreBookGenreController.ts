...
export class RestoreBookGenreController implements Controller {
  async handle(c: Context<App>): Promise<Response & TypedResponse<ControllerResponse, StatusCode, "json">> {
    try {
      const id = c.req.param("id");
      const services = c.get("services");

      await services.bookGenres.restore.execute(id);

      return c.json(
        {
          data: null,
          message: "Book genre relationship was restored successfully",
        },
        HttpStatusCodes.OK,
      );
    } catch (error) {
      ...
      if (error instanceof BookGenreIsActive)
