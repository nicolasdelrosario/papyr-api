...
export class RestoreUserController implements Controller {
  async handle(c: Context<App>): Promise<Response & TypedResponse<ControllerResponse, StatusCode, "json">> {
    try {
      ...
      if (error instanceof UserActive) return c.json({ data: null, message: error.message }, HttpStatusCodes.FORBIDDEN);
