...
import { UserIsNotActive } from "../../domain/exceptions/UserIsNotActive";

export class SoftDeleteUserController implements Controller {
  async handle(c: Context<App>): Promise<Response & TypedResponse<ControllerResponse, StatusCode, "json">> {
    try {
      const id = c.req.param("id");
      const services = c.get("services");

      await services.users.softDelete.execute(id);
