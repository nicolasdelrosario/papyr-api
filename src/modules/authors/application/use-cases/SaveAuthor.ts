  ...
  async execute(author: SaveAuthorDto): Promise<void> {
    ...
    if (isUpdate) {
      ...
      const updatedAuthor = new Author(
        ...
        new AuthorBirthDate(author.birthDate ? new Date(author.birthDate) : existingAuthor.birthDate.value),
        new AuthorDeathDate(author.deathDate ? new Date(author.deathDate) : existingAuthor.deathDate.value),
        new AuthorNationality(author.nationality ?? existingAuthor.nationality.value),
        new AuthorPhotoUrl(author.photoUrl ?? existingAuthor.photoUrl.value),
    ...
    const newAuthor = new Author(
      ...
      new AuthorBirthDate(new Date(author.birthDate)),
      new AuthorDeathDate(author.deathDate ? new Date(author.deathDate) : null),
      new AuthorNationality(author.nationality ?? null),
      new AuthorPhotoUrl(author.photoUrl ?? null),
